...
function deleteDeplicateTableLectureData(specificDay, specificTime, tableData, selectedLectures) {
...
export default async function CombineCurrentDataWithSelectedData(selectedLectures) {
  try {
    ...
    if (storedPlainTableData == null && otherLectureData == null ) {
      ...
      storedPlainTableData = storedPlainTableData ? JSON.parse(storedPlainTableData): [];
      if (storedPlainTableData != null) {
        ...
        day.filter(aDay => {
          switch (aDay) {
            ...
            default:
              time.filter(period => {
                deleteDeplicateTableLectureData(aDay, period, allStoredLectureData, selectedLectures);
              })
              break;
          }
        });
